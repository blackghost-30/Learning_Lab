...
## 1.PWR简介
- PWR：Power Control，即电源控制；
- PWR负责管理STM32内部的电源供电部分，可以实现：
  - **可编程电压监测器（PVD）功能；**
  - **低功耗模式的功能；**
- **可编程电压监测器（PVD）：**
  - 可以监控VDD电源电压，当VDD下降到PVD阀值以下或上升到PVD阀值之上时，PVD会触发中断；
  - 用于执行紧急关闭任务；
- **低功耗模式：**
  - **包括睡眠模式（Sleep）、停机模式（Stop）和待机模式（Standby）；**
...
- **模拟部分供电：**
  ...
  - VDDA，为模拟部分供电；
  - 包括AD转换器、温度传感器、复位模块、PLL锁相环等，AD转换其还有两根参考电压引脚；
  - 这些模块的供电正极是VDDA，负极是VSSA；

- **数字部分供电：**

  - 即中间部分的供电区域；
  - 包括VDD供电区域和1.8V供电区域；
  - VDD供电区域包括**待机电路、唤醒逻辑、电压调节器、看门狗**等；
  - 1.8V供电区域包括**CPU核心、存储器、内置数字外设**等；
  - **1.8V供电区域当需要使用时才通过IO电路转换到3.3V，否则由电压调节器提供1.8V供电；**

- 后备供电：

  - 即VBAT引脚部分；
  - 包括LSE的32K晶体振荡器、后备寄存器、RCC的BDCR寄存器、RTC；
  - 低电压检测器可控制开关，VDD有电时由VDD供电，否则由VBAT供电；

![电源框图](images/44.第四十四节课_PWR电源控制/电源框图.png)



## 3.上电复位和掉电复位以及可编程电压检测器PVD

### 3.1 上电复位和掉电复位

- 即当VDD或VDDA电压过低时，内部电路直接产生复位，让STM32复位住，不要乱操作；
- **上升沿是1.92伏，下降沿是1.88伏，中间有40mV的迟滞带；**

![上电复位和掉电复位](images/44.第四十四节课_PWR电源控制/上电复位与掉电复位.png)

### 3.2 可编程电压检测器PVD

- 跟上面差不多，但是它的阈值电压是可以使用程序指定的；
- 可参考数据手册获得其描述；
- **PVD检测到电压有问题后会产生中断，且是外部中断，直接通往EXTI边沿检测器，若要使用中断记得打开外部中断使能；**
- **RTC本身有中断直接接到NVIC，但是因为只有外部中断可以唤醒停止模式，所以RTC也有一条通往外部中断的线；**

![PVD](images/44.第四十四节课_PWR电源控制/可编程电压检测器.png)
...
- 进入：
  - 通过**WFI**(Wait For Interrupt，等待中断)或**WFE**(Wait For Event，等待事件)即可进入；
  - 这两条是内核的指令，可直接调用函数；
- 唤醒：
  - 前者唤醒条件为**任一中断**，唤醒后进入中断函数；
  - 者唤醒条件为**唤醒事件**，唤醒后可以直接从暂停的地方开始运行；
- **对电路的影响：只把CPU时钟关了**
  - 1.8V区域：进入睡眠后CPU的时钟关，对其他时钟和ADC时钟无影响，即**CPU不运行但外设运行**；
  - 对VDD区域时钟无影响；
  - 对电压调节器的操作是开；
- 唤醒后的动作：
  - 唤醒后从**暂停的地方**开始运行；
  - **SLEEPONEXIT位**决定是立刻睡眠还是处理完中断再睡眠（睡眠模式的细分）；
  - 在睡眠模式下所有的IO引脚都保持**运行模式时的状态**；

![睡眠模式总结](images/44.第四十四节课_PWR电源控制/睡眠模式总结.png)

### 4.2 **停机模式**

- 进入：
  - PDDS的设置：设置PDDS=0；
  - LPDS的设置：1则电压调节器进入低功耗，0则开启；
  - SLEEPDEEP的设置：设置为1；
  - **设置好上述位之后调用WFI或WFE进入停机状态；**
- 唤醒：
  - WFI只有**任一外部中断（注意有4个借道的外设）**才可以唤醒；
  - WFE对应的只有**外部中断的事件**才可以唤醒；
- 对电路的影响：
  - 关闭所有1.8V区域的时钟，HSI和HSE的振荡器关闭，所以所有外设都不可以运行；
  - 但是**两个低速时钟LSI和LSE**继续运行；
  - 电压调节器处于开启或是低功耗模式，所以**CPU和存储器的数据还保持**；
- 唤醒后的动作：
  - 唤醒后从**暂停的地方**开始执行（因为还有数据，所以可以直接运行）；
  - 当退出停止模式后**HSI被选为系统时钟**；
  - 当电压调节器处于低功耗模式下，系统退出时会有一段额外的启动延时；
  - 在停机模式下所有的IO引脚都保持**运行模式时的状态**；

![停机模式总结](images/44.第四十四节课_PWR电源控制/停机模式总结.png)
...
- 进入：
  ...
  - **设置好上述位之后调用WFI或WFE进入停机状态；**
- 唤醒：
  - **WKUP引脚的上升沿、RTC闹钟事件、NRST引脚上的外部复位、IWDG复位才可以唤醒**；
  - 普通外设的中断和外部中断都无法唤醒；
- 对电路的影响：
  - 关闭所有1.8V区域的时钟；
  - HSI和HSE的振荡器关闭，**但是两个低速时钟继续运行用于维持RTC和独立看门狗的运行**；
  - 电压调节器关闭，所以数据丢失；
- 唤醒后的动作：
  - 退出后**从头开始运行**（因为没有数据了）；
  - 在待机模式下所有的IO引脚变为**高阻态即浮空输入**；

![待机模式总结](images/44.第四十四节课_PWR电源控制/待机模式总结.png)

![低功耗模式汇总](images/44.第四十四节课_PWR电源控制/低功耗模式汇总.png)
...
- **配置其他寄存器都需要在这之前，但是写程序时调用函数即可将其他寄存器的位设置好**；

![低功耗模式的切换选择](images/44.第四十四节课_PWR电源控制/低功耗模式的切换选择.png)
...
注意，任何一种低功耗模式下直接下载程序都是不行的，需要按住复位键然后下载再松手复位键才能下载。

## 1.修改主频——system_stm32f10x.c与.h文件

### 1.1 接线图

- 直接接个OLED屏幕即可；

![修改主频](images/45.第四十五节课_修改主频_睡眠模式_停止模式_待机模式/13-1_修改主频.jpg)

### 1.2 system_stm32f10x.c文件

- 作用：用于**配置系统时钟，就是配置RCC时钟树**；

- 文件有两个外部可调用函数和一个外部可调用变量：SystemInit()、SystemCoreClock、SystemCoreClockUpdate()

  - **SystemInit()**：用于配置时钟树，一进来由启动文件调用即可配置好时钟，若启动失败就用内部的HSI的8MHz；
  - **SystemCoreClock**：变量表示当前主频的值；
  - **SystemCoreClockUpdate()**：用于更新SystemCoreClock，因为这个变量只有一开始的一个赋值，后面更改主频是不会跟着变化的；

  ![system_stm32f10x.h文件声明内容](images/45.第四十五节课_修改主频_睡眠模式_停止模式_待机模式/system_stm32f10x.h文件声明内容.png)

### 1.3 项目编程

- 在OLED显示屏的基础进行修改；

- 主频的更改：解除宏定义的注释即可，但是需要先将**只读解除**；

![主频的修改](images/45.第四十五节课_修改主频_睡眠模式_停止模式_待机模式/主频的修改.png)

- 系统时钟初始化的过程：

  - 上电后SystemInit()函数执行，首先启动HSI，然后恢复外设的缺省配置，最后调用SetSysCloc()函数；
  - SetSysClock()函数是一个分配函数，根据我们解除的宏定义，选择执行不同的配置函数；
  - 在不同的配置函数内部完成时钟的初始化；

- 项目编程

  - 修改主频实现闪烁字符串的周期不同的效果；
  - 在延时模块中，如果想即使改变主频延时也不出错的话，就别写死，而是用SystemCoreClock变量来计算；
  - 只需在main.c文件中编程，其他模块均不需要封装和修改，除了system_stm32f10x.c文件中的修改主频；
...
- RXD接在PA9，TXS接在PA10，与串口那节的代码一样；

![接线图](images/45.第四十五节课_修改主频_睡眠模式_停止模式_待机模式/13-2_睡眠模式+串口发送+接收.jpg)

### 2.2 项目编程

- 在**”9-2 串口发送+接收“**项目的基础上进行修改；

- 只能用睡眠模式串口才能接收和发送数据，因为其他模式下串口外设都是不工作了的；
- 只需要在while循环最后加入**WFI的指令**即可；
- **代码的执行流程如下：**
  - 上电后执行一次循环，没有数据所以显示一下字符串，接着进入睡眠模式；
  - 串口发来数据，产生外部中断，程序执行中断函数并并接收到数据，然后接着从WFI()位置运行；
  - 即继续运行循环状态，读到了数据，显示并闪烁字符串，然后重新进入睡眠状态；
- 只需在main.c文件中编程：
...
- 与外部中断那节的接线一样，红外传感器的DO输出接到STM32的PB14号脚；

![接线图](images/45.第四十五节课_修改主频_睡眠模式_停止模式_待机模式/13-3_停止模式+对射式红外传感器计次.jpg)

### 3.2 项目编程

- 在**对射式红外传感器计次**的项目基础上修改；
- **中断可以在没有时钟的情况下工作，所以这里使用了停止模式来对对射式红外传感进行计次**；
- 睡眠模式只是对内核的操作，不需要PWR的库函数，但是停止模式和待机模式都需要PWR库函数；
  - **PWR_WakeUpPinCmd()：WKUP引脚使能函数，只需要使能即可，无需再初始化GPIO；**
  - **PWR_EnterSTOPMode()**：进入停止模式的函数，内部已经有__WFI()函数了；
  - **PWR_EnterSTANDBYMode()**：进入待机模式的函数；

![PWR相关库函数](images/45.第四十五节课_修改主频_睡眠模式_停止模式_待机模式/PWR相关库函数.png)

- 只需要在main.c文件中编程即可，即基本执行流程与上面的睡眠模式相同：
...
- WKUP引脚的PA0接一条线，手动切换0V和3.3V；

![接线图](images/45.第四十五节课_修改主频_睡眠模式_停止模式_待机模式/13-4_待机模式+实时时钟.jpg)

### 4.2 项目编程

- 在**实时时钟项目**的基础上进行修改；
- 项目的流程目的：
  - 设置RTC闹钟：RTC_SetAlarm()函数用于设定闹钟值，CNT与该值相等时将触发闹钟事件；
  - 进入待机模式；
  - 使用闹钟信号，唤醒待机模式；
- **待机模式与前面模式的区别：**
  - 待机模式被唤醒后是从main()函数从头运行的，所以会调用SystemInit()函数配置时钟；
  - 而前面两个只是从while循环开始执行而已；
- 只需要在main.c文件中编程即可：
...
## 1.WDG简介
- 简介：WDG（Watchdog），即看门狗；
- 作用：
  - 看门狗可以监控程序的**运行状态**；
  - 当程序因设计漏洞、硬件故障、电磁干扰等原因，出现卡死或跑飞现象时，看门狗能及时**复位程序**；
  - 这样就可以**避免程序陷入长时间的罢工状态**，保证系统的可靠性和安全性；
- 工作逻辑：
  - 看门狗本质上是一个**定时器**；
  - 指定时间范围内，程序没有执行**喂狗（重置计数器）操作**时，看门狗**硬件电路**就自动产生复位信号；
- STM32内置两个看门狗
  - **独立看门狗（IWDG）**：独立工作，即其时钟是**专用的LSI内部低速时钟**，对时间精度要求较低；
...
- **框图介绍：**
  - 类似于定时器的时基单元；
  - 也是包括了预分频器、计数器、重装载寄存器
- **预分频器：**
  - 预分频器的输入时钟为LSI，低速时钟，频率为40KHz；
  - 预分频器为8位，最大进行256分频；
  - 预分频器的分频系数由预分频寄存器IWDG_PR决定；
- **计数器：**
  - 计数器为**自减计数器**，每来一个时钟自减1；
  - 计数器是12位的，所以最大值为4095，自减到0后产生**IWDG复位**；
- **重装载寄存器：**
  - 重装寄存器即RLR，预先写好值；
  - 在运行过程中，在**键寄存器**里写一个特定数据控制电路进行喂狗，重装值就会复制到当前的计数器中；
- **状态寄存器：**
  - SR，标志电路运行的状态；
- **供电分配：**
  - **看门狗的下方框图外设处于VDD供电区，停机和待机模式时仍能正常工作；**
  - 上框图的寄存器是工作在1.8V供电区的；

![IWDG框图](images/46.第四十六节课_WDG看门狗/IWDG框图.png)

### 2.2 IWDG键寄存器

- 键寄存器本质上是**控制寄存器**，用于控制硬件电路的工作；
- **采用键寄存器的原因：**
  - 在可能存在干扰的情况下，采用控制寄存器的某一位控制看门狗其出错概率可能较大；
  - 在电磁干扰下，控制寄存器某一位可能由0变1或由1变0，产生一些不可预见的BUGGER；
  - 通过在整个键寄存器写入特定值来代替控制寄存器写入一位的功能，可降低硬件电路受到干扰的概率；
- 键寄存器的控制逻辑：
  - **写入0xCCCC则启动独立看门狗；**
...
![键寄存器总结](images/46.第四十六节课_WDG看门狗/键寄存器表格总结.png)

### 2.3 IWDG超时时间

- 超时时间：TIWDG = TLSI × PR预分频系数 × (RL + 1)；
- 其中：TLSI = 1 / FLSI，TLSI是固定的**0.025ms**；

![IWDG超时时间](images/46.第四十六节课_WDG看门狗/IWDG的超时时间.png)
...
- **时钟源与预分频器：**
  - **时钟源为PCLK1（APB1，36MHz）**，再经过预分频器WDGTB；
  - **WDGTB和IWDG的PR以及定时器PSC都是一个东西；**
- **控制寄存器以及计数器：**
  - 6位自减计数器，且与SR寄存器共用寄存器，即看门狗控制寄存器**WWDG_CR寄存器**；
  - **T6为溢出标志位**，不作为自减计数器的一部分，为1时表示计数器没溢出；
  - **把T6位当作计数器一部分时，则减到0x40时就是复位；否则就是低6位减到0x00时复位；**
  - 且还有一个WDGA位，是**窗口看门狗的使能位**；
- **重装载寄存器：**
  - 没有重装寄存器，CNT的值由自己写入；
- **配置寄存器：**
  - 即看门狗控制寄存器**WWDG_CFR**；
  - **最早喂狗时间：**
    - 由WWDG_CFR的数据决定；
    - WWDG_CFR的值和WWDG_CR中的值一直做比较，当要喂狗时会写入WWDG_CR，此时与门打开；
    - 若WWDG_CFR的值大于写入WWDG_CR中的值，两个1通过与门就会输出复位；
  - 最晚喂狗时间：
    - **最晚的喂狗界限由CNT决定，CNT减到0时复位；**

![WWDG框图](images/46.第四十六节课_WDG看门狗/WWDG框图.png)

### 3.2 WWDG工作特性

- 递减计数器T[6:0]的值小于**0x40**时，WWDG产生复位，这里是T6也为计数器的位；
- 递减计数器T[6:0]在窗口**W[6:0]外被重新装载时**，WWDG产生复位，即不能过早喂狗；
- 早期唤醒中断EWI：
  - 递减计数器T[6:0]等于0x40时可以产生早期唤醒中断（EWI）；
  - 用于重装载计数器以避免WWDG复位；
  - 这个中断就在溢出的前一刻产生，称为**“死前中断”**；
- 定期写入WWDG_CR寄存器（喂狗）以避免WWDG复位；

![WWDG工作特性](images/46.第四十六节课_WDG看门狗/WWDG的工作特性.png)

### 3.3 WWDG超时时间

- **超时时间（喂狗最晚时间）：**
  - TWWDG = TPCLK1 × 4096 × WDGTB预分频系数 × (T[5:0] + 1)；
  - 4096是因为时钟进来后先进行**4096分频（框图里面没画出来但是是固定有读到）**再进入到预分频器；
  - 预分频系数只能是1、2、4、8；
- **窗口时间（喂狗最早时间）：**
  - TWIN = TPCLK1 × 4096 × WDGTB预分频系数 × (T[5:0] - W[5:0])；
- 其中：TPCLK1 = 1 / FPCLK1；

![WWDG超时时间](images/46.第四十六节课_WDG看门狗/WWDG的超时时间.png)
...
- 一般用独立看门狗就已经足够了；
- **无论是IWDG和WWDG，开启后都是无法关闭的；**

![IWDG和WWDG的区别对比](images/46.第四十六节课_WDG看门狗/IWDG和WWDG的对比区别.png)
...
![接线图](images/47.第四十七节课_独立看门狗与窗口看门狗/14-1_独立看门狗.jpg)

### 1.2 独立看门狗的配置流程

- 第一步：开启LSI的时钟
  - 但是如果我们打开了看门狗那LSI时钟会被强制打开；
  - 等LSI稳定后，就可以自动为独立看门狗提供时钟了，所以这一步不需要代码来执行；
- 第二步：解除写保护
- 第三步：写入预分频器和重装寄存器；
- 第四步：启动看门狗；

### 1.3 IWDG相关库函数——stm32f10x_iwdg.h

- IWDG_WriteAccessCmd()：写使能控制函数；
- IWDG_SetPrescaler()：写预分频器；
- IWDG_SetReload()：设置重装值；
- IWDG_ReloadCounter()：即喂狗；
- IWDG_Enable()：启动独立看门狗；
- IWDG_GetFlagStatus()：获取标志位状态；
- **RCC_GetFlagStatus()：RCC外设中的获取标志位函数，可以查看是因为什么原因导致的复位；**

![IWDG相关函数](images/47.第四十七节课_独立看门狗与窗口看门狗/IWDG相关库函数.png)

### 1.4 项目编程

- 在OLED显示屏的基础上修改；
- 只需在main.c文件中编程即可，无需封装模块：
...
![接线图](images/47.第四十七节课_独立看门狗与窗口看门狗/14-2_窗口看门狗.jpg)
